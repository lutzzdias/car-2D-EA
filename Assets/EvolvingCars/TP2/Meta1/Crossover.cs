...
namespace GeneticSharp.Runner.UnityApp.Commons
{
    public class Crossover : ICrossover
    {
        ...
        public IList<IChromosome> Cross(IList<IChromosome> parents)
        {
            ...
            float crossoverProbValue = RandomizationProvider.Current.GetFloat();

            if (crossoverProbValue <= crossoverProbability)
            {
                // perform crossover from the start up to the cutPoint
